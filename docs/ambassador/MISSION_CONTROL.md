...
3. Supply both audited databases for first start through the server-side, gzip/base64 seed variables and matching raw SHA-256 variables. The service writes them atomically only when the `/data` targets do not exist, verifies each digest, then continues from the persistent volume. Remove the seed variables after the first verified volume backup. `outreach.db` remains the source ledger; `mission_control.db` remains its operational projection.
